...
#define EVALLOG "eval.%03d.log"
#define EVALUCLOG "eval.uc.%03d.log"
...
obj_eval(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    switch (iOutput)
    {
        case OUTPUT_FILE:
            cntFile++;
            snprintf(evalUCFile, 255, EVALUCLOG, cntFile);
            snprintf(evalFile, 255, EVALLOG, cntFile);

            fOutUC = fopen(evalUCFile, "wb");
            if (fOutUC != NULL)
            {
                fputc(0xFF, fOutUC);
                fputc(0xFE, fOutUC);
        ...
        case OUTPUT_HEX:
            OutputHex(s, n, "eval", 1);
            break;

        case OUTPUT_ASCII:
            OutputASCII(s, n, "eval", 1);
            break;

        case OUTPUT_DUMP:
            OutputDump(s, n, "eval", 1);
            break;

        case OUTPUT_HEX_ONLY:
            OutputHex(s, n, "eval", 0);
            break;

        case OUTPUT_ASCII_ONLY:
            OutputASCII(s, n, "eval", 0);
            break;

        case OUTPUT_DUMP_ONLY:
            OutputDump(s, n, "eval", 0);
            break;
    }
